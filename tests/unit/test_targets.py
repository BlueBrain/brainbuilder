...
def test_create_node_sets():
    ...
    expected = {'All': {'population': 'not-default'},
    ...
    hierarchy = {'id': 0, 'name': 'a', 'acronym': 'a',
                 'children': [
    ...
    atlas = MockAtlas(hierarchy,
                      {('data', '{S1HL-cylinder}', voxcell.ROIMask): mask
                       })
    res = tested.create_node_sets(cells, full_hierarchy, atlas, targets, allow_empty, population)
    ...
    expected = {'All': {'population': 'not-default'},
                ...
                'c': {'region': 'c',},
                'a': ['a-only', 'b', 'c'],
                'L3_MC': { 'mtype': 'L23_MC', 'region': { '$regex': '.*3', }, },
                'L5_CUSTOM': { 'layer': '5', },
                'L5_TPC': { 'mtype': [ 'L5_TPC:A', ], },
                'cylinder': {
                    'node_id': [ 0, 1, 2, ],
                    'population': 'not-default',
                    },
